...
static long getbig(const void *data,int off)
{
...
static int nfs_lookup(const char *input,int len,char *output)
{
    ...
    {
        *(unsigned *)output=0;
        if(memcmp(input,fs_getroot(),32))
        {
            i=fs_getparent(input,output+4,&dummy);
            if(!i) i=fs_getattr(output+4,output+36);
        }
        else
        {
            memset(output+4,0,32);
            i=fs_getattr(input,output+36);
        }
        if(i)
        {
            *(unsigned *)output=htonl(i);
...
static int nfs_readdir(const char *input,int len,char *output)
{
    ...
    unsigned cookie;
    char dummy[32];
    /* 0..0xF - Reserved for internal "." handling  */
    memcpy(&cookie,input+32,4);
    /* "." */
    if(!cookie)
    {
        strcpy(output+16,".");
        i=fs_getfileid(input,(unsigned *)(output+8));
        cookie=1;
    }
    /* ".." */
    else if(cookie==1)
    {
        strcpy(output+16,"..");
        if(memcmp(fs_getroot(),input,32))
            i=fs_getparent(input,dummy,(unsigned *)(output+8));
        else
        {
            memcpy(output+8,"\0\0\0\0",4);
            i=0;
        }
        cookie=16;
    }
...
{
    unsigned tsize,bsize,blocks,data[3];
    memcpy(output,"\0\0\0\0",4);
    memcpy(output+16,"\0\0\0\0\0\0\0\0",8);
    fs_statfs(&tsize,&bsize,&blocks);
    while(tsize>4096) tsize>>=1;
    data[0]=htonl(tsize);
    data[1]=htonl(bsize);
    data[2]=htonl(blocks);
    memcpy(output+4,data,12);
    return 24;
}
