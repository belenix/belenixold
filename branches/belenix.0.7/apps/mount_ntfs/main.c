...
int main(int argc,char **argv)
{
    ...
    for(i=1,err=0;i<argc && !err;i++)
    {
        ...
        else if(!strcmp(argv[i],"-dosnames")) dosnames=1;
	...
	/* p5 and greater indicate virtual devices */
	if (partnum > 4) {
            char *s;
            char buf[PATH_MAX];
...
devline_ok:
    ...
    i=ntfs_initialize(image,fsoff,partmode,dosnames);
    if(i) return i;
    ...
    while(1)
    {
        ...
        if(pfd.revents&POLLIN)
        {
            ...
            if(i>0) handle_nfs_packet(sock,&soa,buf.buf,i);
        }
    }
