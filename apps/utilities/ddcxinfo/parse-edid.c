...
/*
 * Mode info to return from parse_edid_detailed if requested
 */
struct vbe_modeline {
...
int
parse_edid_detailed( byte* edid, struct vbe_modeline **modes );


struct vbe_modeline *
parse_timing_description( byte* dtd, struct vbe_modeline *mode );
...
parse_edid_detailed( byte* edid , struct vbe_modeline **modes )
{
  ...
  byte checksum = 0;
  char *vendor_sign;
  int ret = 0, dcount = 0, j = 0;
  
  for( i = 0; i < EDID_LENGTH; i++ )
    checksum += edid[ i ];

  if (  checksum != 0  ) {
      MSG( "EDID checksum failed - data is corrupt." );
      return (-1);
  ...
  /*
   * Determine if we have a Detailed Timing block
   * else we need to use default values.
   */
  ret = -1;
  ...
  /*
   * Build the list of modes and return to caller
   */
  if (modes != NULL) {
    *modes = (struct vbe_modeline *)malloc(sizeof(struct vbe_modeline) * (dcount + 1));
    ...
    for( i = 0; i < NO_DETAILED_TIMING_DESCRIPTIONS; i++,
	   block += DETAILED_TIMING_DESCRIPTION_SIZE )
      {
        ...
        if ( block_type( block ) == DETAILED_TIMING_BLOCK ) {
	  parse_timing_description( block, (*modes)+j );
	  j++;
	}
      }
    ((*modes)+dcount)->refresh = 0;
  ...
  for( i = 0; i < NO_DETAILED_TIMING_DESCRIPTIONS; i++,
	 block += DETAILED_TIMING_DESCRIPTION_SIZE )
    {
      ...
      if ( block_type( block ) == DETAILED_TIMING_BLOCK )
	parse_timing_description( block, NULL );
    }
...
parse_timing_description( byte* dtd, struct vbe_modeline *mode )
{
  ...
  if (mode != NULL) {
    mline = (char *)malloc(512);
    mode->width = H_ACTIVE;
    mode->height = V_ACTIVE;
    mode->interlaced = INTERLACED;
    mode->refresh = ((double)PIXEL_CLOCK)/((double)vtotal*(double)htotal);
    snprintf(mline, 512, 
        "ModeLine \"%dx%d\"  %f  %u %u %u %u  %u %u %u %u  %s %sHSync %sVSync",
         H_ACTIVE, V_ACTIVE, (double)PIXEL_CLOCK/1000000.0,
         H_ACTIVE, H_ACTIVE+H_SYNC_OFFSET,
         H_ACTIVE+H_SYNC_OFFSET+H_SYNC_WIDTH, htotal, V_ACTIVE,
         V_ACTIVE+V_SYNC_OFFSET, V_ACTIVE+V_SYNC_OFFSET+V_SYNC_WIDTH,
         vtotal,
         INTERLACED ? "\"Interlace\" ": "",
         HSYNC_POSITIVE ? "+": "-",
         VSYNC_POSITIVE ? "+": "-");
    mode->modeline = mline;
    mode->hfreq = (double)PIXEL_CLOCK/((double)vtotal*(double)htotal);
    mode->vfreq = (double)PIXEL_CLOCK/(double)(htotal*1000);
