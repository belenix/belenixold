...
static const unsigned char *get_cluster(long long cluster)
{
    int i,oldest;
    unsigned ksusage;
...
static long long rel_to_cluster(const void *runs,long long rel)
{
...
static unsigned getlittle16(const void *data,int off)
{
    const unsigned char *x=(const unsigned char *)data;
    return x[off+1]*0x100+x[off];
}
static unsigned getlittle32(const void *data,int off)
{
    const unsigned char *x=(const unsigned char *)data;
    return x[off+3]*0x1000000+x[off+2]*0x10000+x[off+1]*0x100+x[off];
}
static long long getlittle48(const void *data,int off)
{
    const unsigned char *x=(const unsigned char *)data;
    return getlittle32(data,off)+(((long long)getlittle16(data,off+4))<<32);
}
static long long getlittle64(const void *data,int off)
{
    const unsigned char *x=(const unsigned char *)data;
...
static const unsigned char *find_attribute(const void *block,int typ)
{
    int p,t,l,maxlen;
...
static int read_attribute(pinode inode,int typ,long long offset,void *dest,
...
static int do_fixups(void *dest,unsigned len)
{
    unsigned pos;
...
static time_t get_time(const void *data)
{
    long long ftime;
...
static pinode create_inode(long long inode,const unsigned char *mftentry,int nofile)
{
    int i,l;
    unsigned fsize;
...
int ntfs_initialize(const char *image,long long offset,int partmode,int dosnames)
{
    ...
    void *pold;
    /* Open the disk */
    dskhandle=open(image,O_RDONLY|O_LARGEFILE);
    if(dskhandle<0)
    {
        fprintf(stderr,"Could not open the disk/image %s.\n",image);
        return 2;
    }
    /* Check for a partition table */
    part_offset=offset;
    llseek(dskhandle,offset,SEEK_SET);
...
static int find_tree_nodes(pinode inode,const unsigned char *entry,int maxlen)
{
...
static int load_tree_nodes(pinode inode)
{
    const unsigned char *ptr;
    int i,maxlen;
...
static int get_next_direntry(pinode inode,unsigned *cookie,
