...
Requires: SUNWdbus-bindings-devel
...
%build
export RPM_OPT_FLAGS="$CFLAGS -D_pk_debug"
...
%pre root
grep polkitg $PKG_INSTALL_ROOT/etc/group > /dev/null
[ $? -ne 0 ] && echo 'polkitg::220:' >> $PKG_INSTALL_ROOT/etc/group
grep polkitu $PKG_INSTALL_ROOT/etc/passwd > /dev/null
if [ $? -ne 0 ]
then
	echo 'polkitu:x:220:220:PolicyKit User:%{_datadir}/empty:' >> $PKG_INSTALL_ROOT/etc/passwd
	chmod u+w $PKG_INSTALL_ROOT/etc/shadow
	echo 'polkitu:NP:::::::' >> $PKG_INSTALL_ROOT/etc/shadow
	chmod u-w $PKG_INSTALL_ROOT/etc/shadow
fi
...
%attr(2755,root,polkitg) %{_libexecdir}/polkit/polkit-resolve-exe-helper
...
%dir %attr (0755, root, sys) %{_sysconfdir}/dbus-1
%dir %attr (0755, root, sys) %{_sysconfdir}/dbus-1/system.d
