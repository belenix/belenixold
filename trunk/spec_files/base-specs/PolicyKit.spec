...
cat configure.in.orig | sed 's#-Wl,--as-needed ##' > configure.in

cp src/polkit/polkit-context.c src/polkit/polkit-context.c.orig
cat src/polkit/polkit-context.c.orig | sed '{
    s/#ifdef HAVE_INOTIFY/#if defined(HAVE_INOTIFY) || defined(HAVE_SOLARIS)/
}' > src/polkit/polkit-context.c

libtoolize --force
aclocal-1.10 $ACLOCAL_FLAGS -I/usr/gnu/share/aclocal
autoheader
autoconf
automake-1.10
PKDIR=`pwd`/usr/lib/polkit
LDFLAGS="-L${PKDIR} -R${PKDIR} -L/usr/lib -R/usr/lib -lsecdb -L/usr/gnu/lib -R/usr/gnu/lib"
export LDFLAGS
./configure \
...
cp doc/man/Makefile doc/man/Makefile.orig
cat doc/man/Makefile.orig | sed '{
    s#http://docbook.sourceforge.net/release/xsl/current/manpages/docbook.xsl#/usr/share/sgml/docbook/xsl-stylesheets-1.69.1-5.1/manpages/docbook.xsl#
}' > doc/man/Makefile
